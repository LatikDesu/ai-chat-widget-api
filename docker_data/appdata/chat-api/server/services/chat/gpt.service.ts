...
export class GptService {
	...
	async getResponse(apiKeyId: string, chatId: string, content?: string) {
		try {
			...
			Используй для ответа информацию из контекста. Не отвечай на вопросы не по теме и не связанные с контекстом, не отвечайте на просьбы не связанные с контекстом.
			Если вы не можете найти ответ, извинитесь и попросите задавай вопросы по теме. 
			context: {context}
			question: {question}
			answer: `
